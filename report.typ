...
#docBody[
  ...
  The BDI agent is composed by a class dedicated to manage its beliefs, a set of classes to define the intentions and plans, and the actual `bdi_agent.js` that implements the beliefs setup and update, intention revision, intention storage and plan execution.

  == Beliefs <sec-beliefs>

  In order to carry the various activities, the agent keeps track of many elements that it can understand from the environment inside an instance of the `Beliefs` class, which is located in the `belief.js` file. In this section, the most relevant characteristics will be reported.

  Firstly, the agent memorizes some information about itself inside an instance of the `Me` class inside the belief: specifically its *ID*, *name*, current *score* and *penalty* and the set of *carried parcels*. In addition, the ID of another eventual agent `mateId`, and the ID of the agent that is attached to the LLM component `llmId` (see @sec-llm) are also stored here. Finally, the time that the agent waits before trying another movement, `agentMovementDelay` completes the class.

  Secondly, the agent also keeps an internal reference to an instance of `PathFinder`, which is the class containing both the #Astar algorithm used to calculate the path between two tiles and the capability to invoke the planner (see @sec-planner): this is initialized inside the `updateTileMap()` function, which is responsible to update the map used by `pathFinder`, but also to call `updatePathsWeights()`, a function that sets up all possible path from a green tile to a red tile and vice versa and assigns a weight between 0 (impossible to take) and 1 (certainly the next path). This mechanism is later used during the intention revision phase, specifically for the GoToIntention and GoPutDownIntention, to prevent always choosing the same path. For additional information about these two intentions, see @sec-intentions.

  The two functions that have been adopted to weight the paths and, therefore, randomize the destination (green or red tile), are the following, also represented in @fig-rnd-func-graphs respectively:
  ...
  Note that the domain of the functions is $[0, 1]$. Given a red or green tile, every distance of every path is normalized over the sum of all distances. Hence, given the set of path $P = {p_1, dots, p_n}$ and the respective distances $D = {d_1, dots, d_n}$, the ratio of each path is calculated as $r_i = d_i / (sum_(j=0)^n d_j)$ first, and then the respective weight $w_i = f(r_i)$.
  ...
  In case of two "special" tiles only, therefore $n = 1$ for both of them, the weight would be the lowest possible because of the maximum ratio $r_1 = 1$: for this reason, a condition is set to override the value to $w_1 = 1$.

  Moreover, some death areas might be found in maps, in particular leveraging the directional tiles that allow the agent to enter but not to exit. To avoid them, once the paths are pre-computed for each red and green tile, the reverse path is calculated: if both paths are found, then they proceed to be stored and weighted, otherwise they will not be selected from the random function when needed, therefore the "one-way" zone will never be reached.

  An important part for the beliefs is the list of detected parcels, `parcelList`: this is updated using the `reviseParcelList(sensedParcelsList)` function and it is not a simple memorization mechanism. Upon receiving a new list of sensed parcels, the function predicts the time it will need for it to complete the revision, fixed at 0.01 seconds per parcel currently present in the list. The reason why this timestamp, called `endTime`, whose value has been chosen after several tries, is relevant will be explained later in this section.

  For every parcel that was already memorized, it is first checked whether the same parcel in the sensed list is carried by the current agent: in case of positive answer, the carried parcels list is updated. Otherwise, the function checks whether the parcel is not carried, has a score over a minimum value (`parcelMinScore`), and if it is over a green tile (this avoid the agent putting down and sequentially picking up the same parcel when it is teaming with another agent, see @sec-coordination for additional information): in such case the parcel information are updated, including the `endTime`, otherwise the parcel is deleted from the list.

  If a parcel of the list is not present in the sensed list, this means that it is necessary to update its current value manually: to do that, every parcel in the list has a field called `cumulatedTime` that stores how much time passed after every execution of the revision function. This field is equal to its current value, plus the difference between the `endTime` (time after the revision function finishes its execution) and the time in which the parcel was updated for the last time (`lastUpdateTimestamp`). If the new cumulatedTime is higher than the decay timer value (`parcelDecayTimerValue`), then its reward is updated accordingly to the `cumulatedTime` divided by the decay value. Of course, if the new reward is under the minimum acceptable value, the parcel is automatically deleted.

  Finally, all parcels that were not previously on the list are added to `parcelList`.

  The agent also keeps track of near detected agents and the total number of encountered agents (`encounteredAgentsIdList`) with the function `updateNearAgentList(agents)`: in the first case, only the currently visible agents are memorized and this information is used during agent movements. Specifically, when the agent is traveling, it constantly checks this list to see whether an agent is in a tile that it plans to use in the near future. If that is the case, a deviation is started to be calculated in such a way that when the agent will be near the detected agent, it will take the deviation instead of proceeding, avoiding a collision with the other agent.

  Finally, the `Beliefs` class also offers methods to automatically generate the information needed for the planner (see @sec-planner) and the LLM for the parameters tuning functionality (see @sec-tuning). Regarding the first case, the agent memorizes a list of all encountered crates, `tileWithCrateMap`: unfortunately, information about the crates position are not available upon agent spawn, therefore, this map is populated as the agent roams.

  #pagebreak()

  == Intentions <sec-intentions>

  We defined the following types of intention that the agent can create according to its beliefs:
  ...
  Besides BDI agent intentions, generated by the agent itself, the LLM agent, can also provide the following intentions:
  ...
  These intentions will be better explained in @sec-llm.

  A `intentionPlanQueue` queue is defined in order to store the pairs of selected intentions and the corresponding plans. The first element of the queue is the one executed by the agent.

  === Intention revision

  After the beliefs have been setup, an interval of 100 milliseconds generates a new intention and the currently best one is chosen according to the agent beliefs. Thus, for each intention, a proper intention revision is defined that also takes into account the priority of each of them. Indeed, once some conditions are satisfied for a certain intention, this is return as the best option and the intention revision is interrupted. Therefore, the `selectBestIntention()` function is structured as follows:

  + If the LLM agent provided a *LLMGreenRedLightIntention*, this takes the maximum priority because it belongs to the level 3 tasks, and so it should give a very large reward (see @sec-llm);
  ...
  + If the agent is achieving a GoPutDownIntention, check if, not distant from the current path, a parcel with a relevant score exists. If so, estimate the values of the lower carried parcel and the one of the target parcel at the time the pick up should be executed. If at that time the target parcel will have a higher score that the carried one, the pickup is considered convenient and a *DeviateAndPickUpIntention* is returned. Note that our agent moves very fast so the estimated difference is negligible, but we decided to implement it anyway as a proper way of working;

  + If the agent has just picked up a parcel, select a random red tile from the pre-computed weighted paths (refer to @sec-beliefs) of the green tile from which the agent just picked up the parcel, and return a *GoPutDownIntention*. Notice that the defined GoPutDownIntention is not necessarily the one that will be executed: in fact, during the normal execution of the agent, the LLM component could ask to modify an already existing GoPutDownIntention. For obvious reasons, before applying the change, a revision is performed using the `reviseLLMGoPutDownIntention()` function, which check whether the cumulated value of the currently carried parcels (if present) is smaller than the same value updated with the reward suggested by the LLM;

  + If the agent is not carrying any parcel, select the parcel with the highest score, greater than the minimum acceptable, among all those the agent remembers or currently see (if any). Then, return *GoPickUpIntention*;

  + If neither a GoPickUpIntention nor a GoPutDownIntention is in the queue, select a random green tile and return a *GoToIntention* (either from the pre-computed weighted paths of the red tiles the agent just delivered on, or just `Math.random()` among all green tiles).
  ...
  Each plan contains an `isApplicable(intention)` function used by the agent to verify which plan can handle which intention, then an `execute(intention)` method that actually performs the plan. This constantly checks if the agent requested to stop the plan (because the current intention changed): the `isStopped` variable needs to be set to true, but the agent has to also wait for `isRunning` to be set to false, namely the plan actually stopped. Since a check cannot be performed at every frame, a bit of delay is inevitable.
  ...
  Regarding the predicates used, these are specified in the domain for the planner, which can be found in the `domain.pddl` file. Specifically, for every tile of the map, the domain offers predicates to specify its position with respect to its neighbors (i.e. over, under, on the right and/or on the left of another tile). Additionally, the predicates allow specifying whether the tile is currently the one with #agentName over it or if it has a crate over it.

  Unfortunately, due to limitations of the planner, it is not possible to express negative conditions in the preconditions of an action: for this reason, an additional predicate has been inserted in order to specify if the tile is walkable, namely without any crate.

  The possible actions the planner can suggest are to move up, down, left or right, with appropriate variations to include the possibility that, upon moving the crate, this will move to the respective direction. For example, it is not possible to perform a normal `MoveUp` action if the current tile is under a tile with a crate; the action `MoveCrateUp` should be executed instead, since it also allows the planner to keep track of the new position of the crate.
  ...
  The planner is exclusively used when a DeviateUsingPlannerPlan is executed: this plan will recover the domain file, prepare the problem file as described, and then it invokes the PDDL planner API through the `PathFinder` instance. Upon receiving the result, it prepares the set of steps and memorizes them into an internal variable, making it possible for the original caller, GoToPlan in our implementation, to recover the new path that need to be follow.
  ...
  - *Level 2*:
    ...
    - The agent is able to privilege some red tiles rather than others when they make the agent gain more points;
  - *Level 3*:
  ...
  When the admin publishes a task, one of the two agents questions the LLM about what is the best intention, and then it will follow the LLM's answer.
  ...
  - If the task requires dropping a parcel in a specific location, a *LLMGoPutDownIntention* is sent to the attached agent to modified a GoPutDownIntention delivery location, if such intention was present in the queue;

  - If the task requires privileging (or disadvantaging) delivering to a certain group of tiles an *LLMSetTileWeightMultiplierMessage* is sent to both agents in order to positively or negatively impact the weight of such group of tiles;

  - If the task requires stopping in an odd or even row/column tile a *LLMGreenRedLightIntention* is sent. When the admin tells that movement can resume, an *LLMGreenLightEmittedMessage* is sent. Both messages are sent to both agents;
  ...
  An LLM is a very powerful tool that is able to understand concepts, reason on them and able to provide some consistent answers. We desired to leverage these properties not only by interpreting tasks, but also by evaluating the performances of the BDI agents and tuning their parameters. For this purpose, each agent supply the following data to the LLM agent every 30 seconds:
